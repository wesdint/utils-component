<template>
    ...
    <transition-group name="list" tag="div" :css="false" @before-enter="beforeEnter" @enter="enter">
      <span class="item" v-for="(day, index) in curWeeks" v-bind:key="index" @click="setSelectedDay(day)">
        <div class="date-num" :class="{'hday': index === 0 || index === 6}">
          <div :class="{'is-today': day.isToday}">{{day.day}}</div>
          <div class="color-999 h20" :class="{'selected': day.isToday}" v-if="day.dayMsg">{{day.dayMsg}}</div>
